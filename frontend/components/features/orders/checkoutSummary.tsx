...
export default function CheckoutSummary({
    ...
    const { cartItems, totalPrice, totalItems } = useCartStore();
        ...
        <div className="rounded-xl border border-border dark:border-dark-border bg-card dark:bg-dark-card p-5 flex flex-col gap-4">
                    ...
                    return (
                        <div key={item.productId} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                            <img
                                src={item.image}
                                alt={item.name}
                                className="h-12 w-12 rounded-lg object-cover border border-border dark:border-dark-border shrink-0"
                            />
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-semibold text-text dark:text-dark-text line-clamp-1">{item.name}</p>
                                <p className="text-xs text-text-secondary dark:text-dark-text-secondary">Qty: {item.quantity}</p>
                            </div>
            ...
            <div className="flex flex-col gap-2 text-sm">
                <div className="flex justify-between text-text-secondary dark:text-dark-text-secondary">
                    ...
                    <span className="font-medium text-text dark:text-dark-text">${subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-text-secondary dark:text-dark-text-secondary">
                    <span>Shipping</span>
                    {shippingCost === 0 ? (
                        ...
                        <span className="font-medium text-text dark:text-dark-text">${shippingCost.toFixed(2)}</span>
                    )}
                </div>
            ...
            <button
                ...
                disabled={isPending || !selectedAddress || cartItems.length === 0}
