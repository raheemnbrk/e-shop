            ...
            <div className="p-8">
                <Link to={'..'} relative="path">
                    <div className="flex gap-2 items-center cursor-pointer textxl font-semibold text-light-text">
                        <FaCircleArrowLeft />
                        <h1>back to all products</h1>
                    </div>
                </Link>
                <div className="grid grid-cols-1 md:grid-cols-2 items-center">
                    <img className="w-64 mx-auto" src={productElement.images[activeImg]} alt={productElement.title} />
                    <div>
                        <h1 className="text-3xl font-bold">{productElement.title}</h1>
                        <h1 className="text-2xl font-bold">${productElement.price}</h1>
                        <h1 className="flex gap-4 text-xl font-bold">
                            <span className="line-through text-light-text">${originalPrice}</span>
                            <span className="text-primary">{productElement.discountPercentage}% off</span>
                        </h1>
                        <h1>{productElement.stock} in stock</h1>

                        <div className="mt-4 flex gap-8">
                            {productElement.images.map((ele, ind) => (
                                <img className={`w-24 ${activeImg === ind ? "border-2" : ""} transition-all duration-200 rounded-lg overflow-hidden cursor-pointer`} src={ele} onClick={() => setActiveImg(ind)} />
                            ))}
                        </div>
                    </div>
                </div>
